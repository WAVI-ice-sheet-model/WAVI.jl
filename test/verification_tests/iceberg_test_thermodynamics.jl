...
Driver routine for iceberg simulation with thermodynamics using WAVI.
Defines parameters, initialises state, and runs n_timesteps.
"""
function iceberg_test_thermodynamics(;end_time=1000.)
...
params = Params(accumulation_rate = accumulation_rate, default_temperature = default_temperature,
                default_temperature_ave = default_temperature_ave, glen_a_activation_energy = glen_a_activation_energy)

geothermal_heat_flux=0.05
thermo_dynamics = QuadraticTemperatureApproximationIcebergTest(geothermal_heat_flux = geothermal_heat_flux)
