...
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/h_mask.bin",h_mask)
h_mask.=ntoh.(h_mask)

u_iszero=Array{Float64}(undef,nx+1,ny);
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/u_iszero.bin",u_iszero)
u_iszero.=ntoh.(u_iszero)

v_iszero=Array{Float64}(undef,nx,ny+1);
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/v_iszero.bin",v_iszero)
v_iszero.=ntoh.(v_iszero)

sigma_grid=Array{Float64}(undef,nσ);
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/sigma_grid.bin",sigma_grid)
sigma_grid.=ntoh.(sigma_grid)

basin_ID=Array{Float64}(undef,nx,ny);
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/basinID.bin",basin_ID)
...
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/bed.bin",bed)
bed.=ntoh.(bed)

h=Array{Float64}(undef,nx,ny);
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/thickness.bin",h)
h.=ntoh.(h) 
...
# viscosity=Array{Float64}(undef,nx,ny,nσ);
 #read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/Inverse_2km_viscosity3D_clip_noNan_BedmachineV3_FULL_stripe_fix.bin",viscosity)
 #viscosity.=ntoh.(viscosity)

 temp=Array{Float64}(undef,nx,ny,nσ);
 read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/temps.bin",temp)
 temp.=ntoh.(temp)

 #check this comes in as all zeros...
# damage=Array{Float64}(undef,nx,ny,nσ);
 # read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/Inverse_2km_damage3D_clip_noNan_BedmachineV3_FULL_stripe_fix.bin",damage)
# damage.=ntoh.(damage)

#all zeros and isn't used?? Does it actually need to be here?
 #weertman_c=Array{Float64}(undef,nx,ny);
 #read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/Inverse_2km_WeertmanC_clip_adjusted_noNan_BedmachineV3_FULL_stripe_fix.bin",weertman_c)
 #weertman_c.=ntoh.(weertman_c)

 #same as accumulation data? Redundant??
 accumulation_rate=Array{Float64}(undef,nx,ny);
 read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/accumulation_data.bin",accumulation_rate)
 accumulation_rate.=ntoh.(accumulation_rate)

#This is from data
 dhdt=Array{Float64}(undef,nx,ny);
 read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/dhdt_data.bin",dhdt)
 dhdt.=ntoh.(dhdt)

 #Hope these aren't actually needed...
 #gu_u=Array{Float64}(undef,nx+1,ny);
 #read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/Inverse_2km_u_velocs_clip_noNan_BedmachineV3_FULL_stripe_fix.bin",gu_u)
 #gu_u.=ntoh.(gu_u)
#
 #gv_v=Array{Float64}(undef,nx,ny+1);
 #read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/Inverse_2km_v_velocs_clip_noNan_BedmachineV3_FULL_stripe_fix.bin",gv_v)
 #gv_v.=ntoh.(gv_v)
...
#= accumulation_rate=Array{Float64}(undef,nx,ny);
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/Inverse_2km_accumulation_clip_noNan_BedmachineV3_FULL_stripe_fix.bin",accumulation_rate)
accumulation_rate.=ntoh.(accumulation_rate)

dhdt=Array{Float64}(undef,nx,ny);
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/Inverse_2km_dhdt_clip_noNan_BedmachineV3_FULL_stripe_fix.bin",dhdt)
dhdt.=ntoh.(dhdt) =#

dhdtaccmask=Array{Float64}(undef,nx,ny);
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/dhdt_acc_mask.bin",dhdtaccmask)
dhdtaccmask.=ntoh.(dhdtaccmask)

udata=Array{Float64}(undef,nx+1,ny);
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/udata.bin",udata)
udata.=ntoh.(udata)

vdata=Array{Float64}(undef,nx,ny+1);
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/vdata.bin",vdata)
vdata.=ntoh.(vdata)

udatamask=Array{Float64}(undef,nx+1,ny);
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/udata_mask.bin",udatamask)
udatamask.=ntoh.(udatamask)

vdatamask=Array{Float64}(undef,nx,ny+1);
read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_2015/Mask_1_2km/vdata_mask.bin",vdatamask)
