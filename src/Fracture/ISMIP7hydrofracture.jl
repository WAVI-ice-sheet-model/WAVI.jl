...
function reconstruct_on_grid(fracture::ISMIP7Hydrofracture,grid::Grid) 
    ...
    isnothing(fracture.ice_shelf_collapse_mask) ? zeros(grid.nx,grid.ny) : fracture.ice_shelf_collapse_mask, 
...
function reconstruct_on_subdomain(fracture::ISMIP7Hydrofracture,grid::Grid,subdomain::NTuple{4,<: Integer}) 
    ...
    fracture.damage_value,
    fracture.partially_floating_cells, 
    size(fracture.ice_shelf_collapse_mask) == size(grid)[1:2] ? fracture.ice_shelf_collapse_mask[x_start:x_end, y_start:y_end] : smb.reference_elevation, 
    fracture.path_to_forcing)
end



function update_fracture_climate_forcing!(fracture::ISMIP7Hydrofracture, grid, clock::Clock)
  @unpack dx = grid
