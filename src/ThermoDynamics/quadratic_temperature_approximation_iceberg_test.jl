struct QuadraticTemperatureApproximationIcebergTest{T <: Real} <: AbstractThermoDynamics
...
"""
QuadraticTemperatureApproximationIcebergTest(; <kwargs>)
...
function QuadraticTemperatureApproximationIcebergTest(;
    ...
    return QuadraticTemperatureApproximationIcebergTest(
...
"""
            update_ice_temperature_grounded_melt_rate!(thermo_dynamics::QuadraticTemperatureApproximationIcebergTest, model::AbstractModel)
...
function update_ice_temperature_grounded_melt_rate!(thermo_dynamics::QuadraticTemperatureApproximationIcebergTest{T}, model::AbstractModel) where {T}
    ...
    for j=1:g3d.nys
        for i=1:g3d.nxs
            if gh.mask[i,j]
                ...
                # use simplified horizontal heat flux for iceberg test case
                horizontal_heat_flux = gh.accumulation[i,j]/params.sec_per_year * gh.θ_ave[i,j] # K*m/s
                ...
                # calculate grounded basal melt rate (should be 0 for iceberg test case -> grounded_fraction=0)
