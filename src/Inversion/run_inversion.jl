...
function run_inversion!(simulation,inversion)
    ...
    while !converged && (i_JKV < inversion.inversion_params.max_JKV_iterations)
      ...
      #  println( "mean of quad_f2 before links is ",mean(inversion.fields.gh.quad_f2[inversion.fields.gh.mask]))
        model_inversion_links!(model,inversion)
      #  println( "mean of  quad_f2 after  is ",mean(inversion.fields.gh.quad_f2[inversion.fields.gh.mask]))

        # get_neumann_velocs
        solve_neumann_velocities!(model,inversion,i_JKV)

        #get_dirichlet_velocs
        solve_dirichelt_velocities!(model,inversion,i_JKV)

        println( "mean of u comp neumann velocs is ",mean(model.fields.gu.u[model.fields.gu.mask] ))
        println( "and v comp of neumann velocs is ",mean(model.fields.gv.v[model.fields.gv.mask] ))        
        println( "mean of dirichelt velocs is ",mean(inversion.fields.gu.u[inversion.fields.gu.mask] ))
        println( "and v comp of dircihlet velocs is ",mean(inversion.fields.gv.v[inversion.fields.gv.mask] ))
        ...
        println( "mean of u bed comp neumann velocs is ",mean(model.fields.gh.ub[model.fields.gh.mask] ))        
        println( "mean of u bed comp dirichlet velocs is ",mean(inversion.fields.gh.ub[model.fields.gh.mask] ))
        println( "mean of u surf comp neumann velocs is ",mean(model.fields.gh.us[model.fields.gh.mask] ))
        println( "mean of u surf comp dirichlet velocs is ",mean(inversion.fields.gh.us[model.fields.gh.mask] ))
       ...
       # println("nnz basal melt rate is" ,count(!iszero, model.fields.gh.basal_melt[:]))
        ...
        ###Do I want to  update just dept averaged viscotiy or all layerS? And what order to update quads in?
        inner_update_viscosity!(model)
        update_av_viscosity!(model)
        update_quadrature_falpha!(model)

        update_JKV!(model,inversion,i_JKV)
        update_JRMS!(model,inversion,i_JKV)
        println("Completed iteration number " ,i_JKV)

    end
    println("JKV is" ,inversion.inversion_output.JKV)
    println("JKV is" ,inversion.inversion_output.JRMS)
    gg=hh
...
function start_guess_β_inversion!(model::AbstractModel, inversion)
    ...
    gh.β .= zeros(gh.nxh,gh.nyh)
    gh.β[gh.grounded_fraction .< 0.5].=inversion_params.βfloating_start
...
#    f2=get_rhs_dirichlet_inverse_data(model,surf_speeds,accumulation_rate,dhdt)
 ...
 #   println("Initial residual norm: ", norm(resid))
    ...
    x02m.=ntoh.(x02m)
    #
    xendm=Vector{Float64}(undef,ni);
    read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_github/WAVI-WAIS-setups/inversion_data/bedmachinev3/full_stripe_fix_8km/Inverse_xeend_guess.bin",xendm)
    xendm.=ntoh.(xendm)
    nirhsd1=gu.ni+gv.ni
    ...
    #residm=get_resid(xendm,Op_combo,f)
   ...
   # println("Matlab relative Residual: ", relative_residualm)
    ...
    f_diff=f.-fm
    println(" nnz f_diff " ,count(!iszero,f_diff[:]))
    index = argmax(f_diff) 
 #   println("   index is" ,index)
 #   println(" f at  index is" ,f[index])
 #   println("fm at index is " ,fm[index])
#    println(" f diff at index is" ,f_diff[index])
  #  println("mean f diff is" ,mean(f_diff))
 #   println("f at first index is " ,f[1:5])
#    println("fm at first index is " ,fm[1:5])

#surf=Array{Float64}(undef,gh.nxh,gh.nyh);
#read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_github/WAVI-WAIS-setups/inversion_data/bedmachinev3/full_stripe_fix_8km/Inverse_surf.bin",surf)
#surf.=ntoh.(surf)

#surf_masked=surf[gh.mask]
#ghs_masked=gh.s[gh.mask]
#surf_diff=surf_masked-ghs_masked
#println(" size surf diff " ,size(surf_diff))
#println(" nnz surf diff " ,count(!iszero,surf_diff[:]))
#println("gh.s at first index is " ,ghs_masked[1000:1005])
#println("surf at first index is " ,surf_masked[1000:1005])
#println("surf_diff at first index is " ,surf_diff[1000:1005])
#println("mean surf_diff is" ,mean(surf_diff))
...
#exitnow=exit_now
   ...
   # if clock.n_iter==1
  #      println("Subbing output at iteration 1")
 #   x_output=x0m
    #elseif clock.n_iter==2
     #   println("Subbing output at iteration 2")
      #  x_output=x02m
  #  end
  #=   ###DEBUGGING!! DELETE LATER!!!
    ...
    set_inversion_pressure!(model,inversion,x_output)
         ...
         println("mean of xD is ",mean(x_output[1:(inversion.fields.gu.ni+inversion.fields.gv.ni)]))
         println("mean of beta2 is ",mean(model.fields.gh.β[model.fields.gh.mask]))
         println("mean of quadf2 is ",mean(model.fields.gh.quad_f2[model.fields.gh.mask]))
...
function solve_neumann_velocities!(model,inversion,clock) 
  ...
  #  println("Size of f1 (rhs) is," ,size(f1))
   ...
   #=  ni=gu.ni+gv.ni
    ...
    #
    x02m=Vector{Float64}(undef,ni);
    read!("/data/hpcdata/users/chll1/WAVI_Initial_Data_github/WAVI-WAIS-setups/inversion_data/bedmachinev3/full_stripe_fix_8km/Inverse_xN2guess.bin",x02m)
    x02m.=ntoh.(x02m) =#
...
function get_start_guess_dirichlet(model,inversion)
 ...
 #   x=[gu.samp_inner*gu.u[:];gv.samp_inner*gv.v[:];  1.0e5*ones(gudata.ni);  -1.0e5*ones(gvdata.ni); -1.0e6*ones(ghdata.n)]
...
function get_rhs_dirichlet_inverse_data(model, inversion)
    ...
    vs_data_centT_crop = gv.crop*vs_data_centT
     ...
     # dhdtacc_data_spread=ghdata.spread*dhdtacc_data
...
@.  tmpui = (params.density_ice*params.g*gu.h[gu.mask_inner]).* tmpui
    ...
    surf_point_check=-((gu.h[gu.mask_inner])).*gu.samp_inner* gu.∂xᵀ*(gh.crop*(surf[:]))
    surf_point_check2=-((gu.h[gu.mask_inner])).*gu.samp_inner* gu.∂xᵀ*(gh.crop*(gh.s[:]))


    surf_alone_point_check=gu.samp_inner*gu.centᵀ*(gh.crop*(gh.s[:]))
...
@.  tmpvi = tmpvi * params.g*(0.5*params.density_ice*hvi^2
    ...
    f2[(gu.ni+1):(gu.ni+gv.ni)] .= tmpvi
      ...
      #  println("number of isnan of f1 is " ,count(isnan.(f1)))
      #  println( "number of isnan of f2 is ",count(isnan.(f2)))
    #    println( "f1 at 817 is",f1[817])
    #    println( "f2 at 817 is",f2[817])
    #     println( "params.density_ice*params.g is " ,params.density_ice*params.g)
     #    println( "gu.s at masked 817 is "  ,u_point_check[817])
     println("size of surf_point_check is " ,size(surf_point_check))
         println( "surf_point_check at masked 817 is "  ,surf_point_check[817])
         println( "surf_point_check at masked 817 is "  ,surf_point_check2[817])
    return rhs
end
...
function update_basal_drag_components!(model::AbstractModel)
   ...
   # gh.τbed .= gh.β .*(gh.av_speed ./ (1.0 .+ gh.quad_f2 .* gh.β))
   # τbed_mag=sqrt.(gh.τx_bed.^2 .+ gh.τy_bed.^2)
   # tau_diff=gh.τbed.-τbed_mag
   # println("nnz in tau diff is " ,count(!iszero, tau_diff[gh.mask]))
   # println("max in tau diff is " ,maximum(tau_diff[gh.mask]))
   # println("min in gtaudiff is " ,minimum(tau_diff[gh.mask]))
    return model
end
...
function update_preBfactor_inversion!(model::AbstractModel,inversion)
   ...
   # @unpack gh_inversion=inversion.fields
    ...
    BPower[gh.grounded_fraction .== 1.0] .= inversion_params.Bpower_grounded
...
function update_JKV!(model::AbstractModel,inversion,clock)
   ...
   # println("   σzzsurf_comp is " ,σzzsurf_comp)
   # println("   τx_surf_comp is " ,τx_surf_comp)
   # println("   τy_surf_comp is " ,τy_surf_comp)
...
# Check validity of the surrounding u and v masks and update h_mask accordingly
  ...
  #  println("size  surf_speed_data_on_h is " ,size( surf_speed_data_on_h))
  #  println("nnz data_and_model_mask is " ,count(!iszero, data_and_model_mask))
  #  println("size gh_mask is " ,size(gh.mask))
